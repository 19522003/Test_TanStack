...
export default function AddStudent() {
  ...
  const errorForm: FormError = useMemo(() => {
    const error = isMatch ? addStudentMutation.error : updateStudentMutation.error
    if (isAxiosError<{ error: FormError }>(error) && error.response?.status === 422) {
      return error.response.data.error
    }
    return null
  }, [addStudentMutation.error, isMatch, updateStudentMutation.error])
  ...
  return (
    <div>
      <h1 className='text-lg'>{isMatch ? 'Add' : 'Edit'} Student</h1>
